            ...
            <header>
                <nav>
                    <ul class="nav nav-pills pull-right register-menu">
                         @if (Auth::check() && Auth::user()->user_type_id != UserType::ADMIN())
                         <div class="dropdown">
                             <button class="btn btn-default dropdown-toggle" type="button" id="menu1" data-toggle="dropdown"><?php if(Auth::user()->user_type_id==UserType::CUSTOMER()) {  echo Auth::user()->customer->first_name . ' ' .Auth::user()->customer->last_name ; } else { echo Auth::user()->professional->prof_first_name;}?>
